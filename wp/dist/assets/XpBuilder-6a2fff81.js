...
return (_ctx, _cache) => {
  return (openBlock(), createElementBlock(Fragment, null, [
    createBaseVNode("div", {
      ref_key: "elmove",
      ref: elmove,
      style: normalizeStyle([unref(style), {"position":"fixed","z-index":"9999"}])
    }, [
      createBaseVNode("p", null, "Drag me! I am at " + toDisplayString(unref(x)) + ", " + toDisplayString(unref(y)), 1)
    ], 4),
    _hoisted_1,
    createBaseVNode("ol", null, [
      (openBlock(true), createElementBlock(Fragment, null, renderList(unref(mylist), (li) => {
        return (openBlock(), createElementBlock("li", null, [
          createBaseVNode("div", null, toDisplayString(li.name), 1)
        ]))
      }), 256))
    ]),
    _hoisted_2,
    createBaseVNode("div", null, "drag elements in the list (" + toDisplayString(unref(mylist).length) + ")", 1),
    createVNode(unref(draggable), {
      modelValue: unref(mylist),
      "onUpdate:modelValue": _cache[0] || (_cache[0] = $event => (isRef(mylist) ? (mylist).value = $event : mylist = $event)),
      "item-key": "id",
      onStart: _cache[1] || (_cache[1] = $event => (isRef(drag) ? drag.value = true : drag=true)),
      onEnd: _cache[2] || (_cache[2] = $event => (isRef(drag) ? drag.value = false : drag=false))
    }, {
      item: withCtx(({element}) => [
        createBaseVNode("div", null, toDisplayString(element.name), 1)
      ]),
      _: 1
    }, 8, ["modelValue"])
  ], 64))
