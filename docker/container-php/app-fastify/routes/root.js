...
module.exports = async function (fastify, opts) {
    ...
    fastify.get('/sql', async function (request, reply) {
        let limit = Math.floor(request.query.limit || 1000)
        // compute execution time
        let start = new Date()
        let oups = Math.floor(Math.random() * 100000)
        ...
        fastify.mysql.query(
            "SELECT * FROM users WHERE id NOT IN ('" + oups + "') ORDER BY id DESC LIMIT " + limit, 
            (err, result) => {
        ...
        // fastify.sqlite.all(
        //     "SELECT * FROM news WHERE id NOT IN ('" + oups + "') ORDER BY id DESC LIMIT " + limit, 
        //     (err, rows) => {
        //         if (err) {
        //             console.error(err)
        //             return
